...
- [x] **Composición Genética Multi-Raza y Cruces Absorbentes Zootécnicos (2026-09-24)** 🧬🐄📐:
  - **Traductor de Fracciones Ganaderas Estándar**: Conversión en tiempo real de decimales a fracciones de la industria ganadera (`50% -> 1/2 o F1`, `75% -> 3/4`, `87.5% -> 7/8`, `93.75% -> 15/16`, `96.875% -> Puro por Cruce (PC)`, `100% -> Puro`, junto con `3/8`, `5/8`, `1/4`, `1/8`, etc.), unificado en `src/engine/genetic_engine.py` y `src/pwa/static/app.js`.
  - **Algoritmo de Cruces Absorbentes (Grading Up / Upgrading)**: Cálculo genético $\text{Cría} = \frac{1}{2}\text{Madre} + \frac{1}{2}\text{Padre}$. Caso zootécnico validado: Vaca 3/4 Gyr (75%) + 1/4 Holstein (25%) cruzada con Toro Brahman puro (100%) genera cría con 50% Brahman (1/2), 37.5% Gyr (3/8) y 12.5% Holstein (1/8), generando automáticamente la etiqueta `1/2 Brahman + 3/8 Gyr + 1/8 Holstein`.
  - **Tabla Relacional y Persistencia Transparente**: Tabla `composicion_racial (animal_id, raza, porcentaje)`. `Database.guardar_composicion_racial` y `obtener_composicion_racial`. Herencia automática al nacer: `Database.registrar_parto` calcula y persiste la composición racial de la cría a partir de madre y padre sin requerir intervención manual.
  - **Ficha Técnica y Modal Interactivo en PWA**: Barra de composición apilada multicolor por raza (verde Brahman, azul Gyr, ámbar Romosinuano, etc.) y chips con fracciones. Modal interactivo con botón `➕ Añadir Raza`, entradas numéricas con traducción instantánea a fracciones, validador de suma en tiempo real (`Suma: 100% ✅` con botón `Completar al 100%` si falta) y guardado reactivo sin recargar la página.
  - **Predicción en Captura Rápida de Partos**: Banner reactivo en el formulario de Parto que anticipa la genética estimada de la cría (`🧬 Genética estimada cría: ...`) en cuanto se seleccionan la madre y el reproductor.
  - **Integración con Tablas Nativas de Software Ganadero (`raza.dbf` y `hoja.dbf`)**: Decodificación de códigos de raza SG (`T` = Taurino, `C` = Cebuino, `I` = Indeterminado, eliminando la ambigüedad que marcaba erróneamente toros taurinos como Cebú) y extracción de grupos raciales `CODGR1..4` y `PORGR1..4` en `import_animales` (931 animales históricos con composición multi-raza migrados a `composicion_racial`, ej. toro `T02`: 50% Girolando + 35% Gyr + 10% Holstein + 5% Cebú).
  - **Validación Automatizada**: 7 pruebas en `tests/test_composicion_genetica.py` (100% passed), 43 pruebas en `test_dbf_importer.py` en verde, verificación de sintaxis JS (`node --check`) y compilación Python completa.
- [x] **Consolidación de Razas, Control Estricto OWNER para Deshacer y Pulido Visual PWA (2026-09-25)** 🧬🛡️📱:
  - **Eliminación de Eventos Exclusiva para OWNER**: Restricción estricta en backend (`/api/eventos/eliminar` retorna `403 Forbidden` para ADMIN y TRABAJADOR) y frontend (botón "Deshacer Registro" y papeleras en tablas solo visibles para rol `OWNER`). Suite de pruebas `test_eliminar_eventos.py` actualizada y 100% verde.
  - **Ícono de Sistema Vectorial Reparado**: SVG de la rueda dentada/engranaje en `#sheet-item-sistema` corregido con el trazado Feather oficial sin deformaciones en celular.
  - **Limpieza de Redundancias en Porcentajes**: Corrección en `recalcularTotales` de `app.js` para evitar impresiones repetidas como `30% -> 30%` en el modal o `30% (30%)` en los chips de la ficha técnica.
