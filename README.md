...
- [x] **Fase 7 — PWA Oficina + Corral Offline (Completa 2026-09-05)**:
  ...
  - **Leche → Finanzas: Foto como Evidencia y Monto Pagado Automático (2026-09-07)**: la foto del recibo ahora se guarda al momento de analizarla con IA, sin depender de llegar hasta "Guardar Quincena" (de paso se corrigió un bug real donde ese guardado manual estaba silenciosamente roto por variables sin definir en su alcance). El parser de visión (`recibo_leche_parser.py`) extrae también `precio_litro` y `valor_total_pagado` directo del recibo (validado contra un recibo real: 5.092 L × $2.000/L = $10.184.000, los 3 valores cuadran) y los precarga editables en la revisión; al presionar "Guardar Quincena" con el monto confirmado, se crea automáticamente el ingreso en Finanzas (`VENTA_LECHE`) con esa misma foto como respaldo.
- [x] Suite de pruebas con pytest: **608 pruebas en verde** (100% pasando).
